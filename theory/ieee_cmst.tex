...
As shown in Table \ref{tab:sll_comparison}, while a $k=2$ window may exhibit a nominal "first side lobe" at -65 dB, the subsequent lobes drop off at an accelerating rate. By the time the frequency offset reaches the "Abyss" ($s > 85$), the side lobe energy has effectively vanished below the limits of double-precision floating-point representation ($-308$ dB).
