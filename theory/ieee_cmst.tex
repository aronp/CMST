...
Numerical experiments confirm that $p=2$ yields optimal sidelobe suppression.
\end{itemize}
...
In contrast, the CMST kernel $\Phi_p(t) = \exp(1+t^p-1/(1-t^{p}))$ possesses no characteristic side lobe level. Because $\Phi_p(t)$ is $C^\infty$ and vanishes to all orders at the analytic wall $|t|=1$, the side lobes do not exhibit the standard $O(s^{-n})$ decay. Instead, the spectral envelope follows a super-polynomial trajectory:
\begin{equation}
|F(s)| \sim O\left(|s|^{-3/4} \exp(-\sqrt{|s|})\right) 
\end{equation}
