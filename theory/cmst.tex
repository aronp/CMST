...
is just a constant, independant of frequency, so the error term is
\begin{equation}
...
This filter effectively reverses the spectral "softness" introduced by the window. Because the Spectral Stiffness $\kappa$ is a deterministic property of the chosen window, this correction is scale-invariant and independent of the sampling frequency $F_s$. In high signal-to-noise ratio (SNR) environments this recovers some of the resolution lost to spectral stiffness.  It should however be noted that applying this filter magnifies noise and there are a couple of approximations in the derivation.  The window shape is not gaussian and our bins are impacted by more than just the adjacent bins.  Its usefulness will depend on the application.
...
arXiv preprint arXiv:0612833, 2006.
