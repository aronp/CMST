...
\begin{theorem}[$\lim_{a \to 1} \mathcal{F}_a(s) = 2 \, \text{Si}(2 s) \sin (s)$]
\end{theorem}
\begin{proof}
\begin{equation}
...
\begin{theorem}[$ \int_{-1}^{1} \exp\left( \frac{1}{t^{2k}-1} \right)cos(st), dt  =-2\,\text{Si}(2 s) \sin (s) \ast \int_{-1}^{1} \exp\left( \frac{1}{t^{2k}-1} \right)cos(st) \, dt $]
\end{theorem}
\begin{proof}
Let
\begin{equation}
...
\begin{equation}
\imath s W(s) = -\imath \, 2 \, s \, \text{Si}(2 s) \sin (s) \ast W(s)
\end{equation}

or
\begin{equation}
W(s) = -2 \, \text{Si}(2 s) \sin (s) \ast W(s)
\end{equation}
\end{proof}
\begin{remark}
...
Since the ratio $a_1/a_0$ define the whole series, it would be nice to get an expression for them.

\section*{Derivation of the CMST Moment Ratio Continued Fraction}
...
\subsection*{4. Final Continued Fraction Expansion}
By iterating this step for $k=1, 2, 3, \dots$, we obtain the continued fraction for the first moment ratio $\frac{a_1}{a_0}$:

\begin{equation}
    \frac{a_1}{a_0} = \cfrac{-1}{\Lambda_1 \left[ 2 + \Omega_1 \cfrac{-1}{\Lambda_2 \left[ 2 + \Omega_2 \cfrac{-1}{\Lambda_3 \left[ 2 + \dots \right]} \right]} \right]}
\end{equation}


For a continued fraction, this converges really slowly to a value $\approx -0.0790568$.  I cant find any other relationships with this number, but it generates our whole FIR.
