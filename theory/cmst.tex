...
\fancypagestyle{preprint}{
    ...
    \fancyhead[C]{\small \textit{Preprint -- January 2026}}
    \fancyfoot[C]{\thepage}
}
...
which  achieves root-exponential spectral decay and is in Gevrey class $\mathcal{G}^2$.
...
We finish by using these functions to produce a $C^\infty$ window function with compact support and super-algebraic sidelobe suppression which belongs to the Gevrey 
class $\mathcal{G}^2$, the maximal smoothness compatible with 
compact support.
...
The function
\[
\psi(t)=\frac{f'(t)}{f(t)}=\sum_{i=1}^n \frac{1}{t-r_i}
\]
is strictly decreasing on each interval $(r_i,r_{i+1})$, with 
$\psi(t)\to +\infty$ as $t\downarrow r_i$ and 
$\psi(t)\to -\infty$ as $t\uparrow r_{i+1}$.
Thus $\psi(t)=c$ has exactly one solution in each such interval.
...
(This follows from (S2) by solving $f+af'=0$ via $\psi(t)=-1/a$.)
...
A \emph{Cosh Moment Sturm Tower} (CMST) is a collection
\[
...
We will refer to $f(t)$ as even if it's an even function and $f(t)$ not even if $f(t) \ne f(-t)$
\end{definition}
...
\begin{theorem}[P\'olya, 1918–1927]\label{thm:polya-cosine}
...
\begin{proof}[Proof sketch]
Pólya’s method places these cosine/Fourier transforms in the Laguerre–Pólya class by constructing them from kernels that are totally positive (``universal factors''). For $e^{-t^{2q}}$ (even, rapidly decaying, log-convex tails), one obtains an entire function of order $2q/(2q-1)\in(1,2)$ with only real zeros. See the survey of Dimitrov–Rusev for a modern consolidation $\cite{dimitrov2011}$.
\end{proof}
...
\item For the monomial case $\psi(t)=e^{-t^{2m}}$ with $m\ge 2$, it is classical (Pólya) that $\widehat{\psi}$ has only real zeros
...
We won't be using these results further on, but if you are here, looking at zeros, their distribution is probably of interest. \newline
...
\begin{remark}
AsymptoticalGW150914ly we should expect the function to behave like $\cos(s z(s))$
...
\end{equation}
near say $s=100$ where the normalisation term is of the order of $\sim 10^{274}$ and the function becomes really sharp.  And since its Dirac delta like, we get that the terms in $s$ have the ratio of roughly the saddle point and since the shape is skewed to the high side each term will increase in ratio over the previous terms.  This results in the terms having a strict Log Concavity. This is not meant to be rigorous, just an insight into whats going on with large exponential type functions.
\end{remark}
...
\begin{remark}
Note that this works for negative odd coefficients as well, we just have to make sure that we don't multiply a function with negative odd coefficients with a function with positive odd coefficients.
\end{remark}
...
\begin{equation}
    \psi(t) = \prod_{n=0}^{\infty} e^{-t^{2nk}} = \exp\left( \frac{1}{t^{2k}-1} \right), \quad t \in (-1, 1)
\end{equation}

and $\psi(t) = 0$ for $|t| \ge 1$. This function is $C^\infty$ and vanishes with all its derivatives at the boundaries $t = \pm 1$.

\begin{theorem}[CMST Membership of the Geometric Kernel]
The function $\psi(t) = \exp\left( \frac{1}{t^{2k}-1} \right)$ belongs to the CMST class on the interval $[-1, 1]$. Consequently, its CMST 
$F_n(s) = \mathcal{C}_1[t^{2n}\psi(t)](s)$ is a interlacing family in 
...
The factorization
\begin{equation}
\psi(t) = \prod_{n=0}^{\infty} e^{-t^{2nk}}  = \exp\left( \frac{1}{t^{2k}-1} \right) 
...
This implies that the window $\psi(t)$ can be interpreted as the infinite intersection of all Hyper-Gaussian stability classes. 
...
\begin{remark}[Construction via the Fundamental Eigenmode]
The construction of the window can be physically interpreted through the eigenmodes of the Fourier-Laplace operator. 
It is a classical result that the Gaussian function $e^{-t^2}$ is the fundamental eigenfunction (ground state) of the Fourier transform, representing the state of minimum uncertainty.

The CMST exponent $\Phi(t)$ is constructed by summing the geometric power series of this fundamental mode:
\begin{equation}
    \Phi(t) = \sum_{n=0}^{\infty} (t^{2k})^n = \frac{1}{1 - t^{2k}}
\end{equation}
By defining the window as $\psi(t) = \exp(-\Phi(t))$, we are effectively creating a "Super-Gaussian" composed of an infinite superposition of higher-order modes.
\begin{enumerate}
    \item \textbf{The Ground State ($n=0$):} Represents the standard Gaussian width.
    \item \textbf{The Infinite Sum ($n \to \infty$):} As the modes pile up, the function transitions from a "soft" bell curve to a "hard" wall at $|t|=1$.
\end{enumerate}
This construction guarantees that the window inherits the smoothness properties of the Gaussian eigenmode (analyticity) while achieving the compact support required for finite-time signal analysis.
\end{remark}
...
\end{equation}
does not converge as $a \to 1$ however if we use the substitution $u = i t$ the integral does converge giving us the analytic continuation.
\begin{equation}
...
\begin{equation}
    I(s) = \frac{\pi}{2} \cosh(s) - 2s \int_{0}^{1} \sinh(st) \arctan(t) \, dt
\end{equation}
...
\begin{remark}
Since the CMST family is closed under multiplication, the energy window is itself a CMST. This implies the Power Spectral Density (PSD) inherits the same super-algebraic decay and resolution properties as the amplitude spectrum, a feature not found in polynomial-based windows
\end{remark}


\section{Differential Structure and Efficient Computation of the CMST Window}

While the integral definition of the CMST window $\psi(t) = \exp\left( \frac{1}{t^2-1} \right)$ ensures its theoretical properties in $C_c^\infty$, its practical application in digital signal processing (such as FIR filter design) benefits from a differential formulation. 
...
Differentiating the kernel $\psi(t)$ with respect to $t$ reveals a distinct non-linear relationship.
\begin{equation}
\psi'(t) = \frac{d}{dt} \exp\left( \frac{1}{t^2-1} \right) = \frac{-2t}{(t^2-1)^2} \psi(t)
\end{equation}
...
To determine the behavior of the Fourier transform $F(s) = \mathcal{C}[\psi](s)$, we map the time-domain operators of \cref{eq:time_ode} to the frequency domain using the standard correspondences $\psi'(t) \leftrightarrow s F(s)$ and $t^n (\cdot) \leftrightarrow i^n D^n (\cdot)$.
...
Since $\psi(t)$ is an even function, $F(s)$ admits a power series expansion of the form:
\begin{equation}
...
For a continued fraction, this converges really slowly to a value 
$\psi \approx -0.0790568$.  I cant find any other relationships with this number, 
but it generates our whole FIR and so it is important here.

\subsection*{5. First few terms}
So if we normalise the first two terms to be 1 and $\psi$ we get the following.
\begin{equation}
\begin{aligned}
a_0 &= 1 \\
a_1 &= \psi \\
a_2 &= \frac{-1 - 16\psi}{120} \\
a_3 &= \frac{2 + 27\psi}{4200} \\
a_4 &= \frac{-157 - 2032\psi}{12,700,800} \\
a_5 &= \frac{10,360 + 133,735\psi}{44,008,272,000}  \\
\cdots
\end{aligned}
\end{equation}
...
\begin{remark}[Relationship of $\psi$]
Noting that if we set $a_0 = 1$ and $a_1 = \psi$ we get

\begin{equation}
\psi = -\frac{\int_{-1}^1 t^2 \exp \left(\frac{1}{t^2-1}\right) \, dt}{2 \int_{-1}^1 \exp
   \left(\frac{1}{t^2-1}\right) \, dt}
\end{equation}
Which is just a case of using the moment transform
\end{remark}
...
\end{equation}
which is the intergral of the Schwartz function.
 \cite{schwartz1950}
\end{theorem}

\begin{remark}
Note that numeric convergence of the above series is hugely dependant on the accuracy of $\psi$.  A 7 decimal place accurate value for $\psi$ blows up around the 5th term for instance.  However if we work out the terms they do telescope giving for instance the first ten terms as
\begin{equation}
\int_{-1}^{1}  \exp\left(\frac{1}{t^2 -1}\right)\, dt 
\approx -\frac{676457349120}{17\, e \, (741069365504297 \psi +58553612934767)} \approx 0.4439530
\end{equation}
which is accurate to about $0.01\%$
...
Because the CMST Window is $C^{\infty}$ and of Gevrey class $\mathcal{G}^2$ the spectral footprint undergoes a root exponential

\begin{theorem}[Super-Algebraic Decay]
Let $\psi(t) = \exp\left(\frac{1}{t^2-1}\right)$ for $|t| < 1$ and $0$ otherwise. The Fourier transform $\hat{\psi}(\omega)$ satisfies:
\begin{equation}
\lim_{\omega \to \infty} |\omega|^N \hat{\psi}(\omega) = 0 \quad \forall N \in \mathbb{N}
\end{equation}
...
\begin{equation}
\hat{\psi}(\omega) = \int_{-1}^{1} \psi(t) e^{-i\omega t} dt
\end{equation}
Integrating by parts once, let $u = \psi(t)$ and $dv = e^{-i\omega t}dt$:
\begin{equation}
\hat{\psi}(\omega) = \left[ \frac{\psi(t) e^{-i\omega t}}{-i\omega} \right]_{-1}^{1} + \frac{1}{i\omega} \int_{-1}^{1} \psi'(t) e^{-i\omega t} dt
\end{equation}
The boundary term vanishes because $\psi(\pm 1) = 0$. Repeating this process $k$ times yields:
\begin{equation}
\hat{\psi}(\omega) = \frac{1}{(i\omega)^k} \int_{-1}^{1} \psi^{(k)}(t) e^{-i\omega t} dt
\end{equation}
Since $\psi(t) \in C_c^\infty(-1, 1)$, all derivatives $\psi^{(k)}(t)$ vanish at the boundaries $t=\pm 1$. The remaining integral is bounded by the maximum of the $k$-th derivative $||\psi^{(k)}||_\infty$. Thus:
\begin{equation}
|\hat{\psi}(\omega)| \leq \frac{2 ||\psi^{(k)}||_\infty}{|\omega|^k}
\end{equation}
Since this holds for any arbitrarily large integer $k$, the decay is super-algebraic.
\end{proof}

\section*{Saddle Point Analysis: Real vs. Imaginary Axis Behavior}

We examine the asymptotic behavior of the Cosh Moment Sturm Transform (CMST) kernel integral as $|s| \to \infty$. The integral is defined as:
\begin{equation}
    I(s) = \int_{-1}^{1} \exp\left( st + \frac{1}{t^2 - 1} \right) dt
\end{equation}

\subsection*{1. The Asymptotic Expansion}
The dominant contribution to the integral arises near the boundary $t = 1$. Let $t = 1 - \epsilon$, where $\epsilon \to 0^+$. The exponent $\Phi(\epsilon)$ can be approximated as:
\begin{align}
    \Phi(\epsilon) &= st + \frac{1}{t^2 - 1} \\
    &\approx s(1 - \epsilon) + \frac{1}{(1-\epsilon)^2 - 1} \\
    &\approx s - s\epsilon - \frac{1}{2\epsilon}
\end{align}
To find the saddle point, we maximize $\Phi(\epsilon)$ with respect to $\epsilon$:
\begin{equation}
    \frac{d\Phi}{d\epsilon} = -s + \frac{1}{2\epsilon^2} = 0 \implies \epsilon_0 = \frac{1}{\sqrt{2s}}
\end{equation}
Substituting $\epsilon_0$ back into the exponent gives the \textbf{Master Equation}:
\begin{align}
    \Phi_{\text{max}}(s) &= s - s\left(\frac{1}{\sqrt{2s}}\right) - \frac{1}{2\left(\frac{1}{\sqrt{2s}}\right)} \\
    &= s - \frac{\sqrt{s}}{\sqrt{2}} - \frac{\sqrt{2s}}{2} \\
    &= s - \sqrt{2s}
\end{align}

\subsection*{2. Case A: The Real Axis (Laplace Domain)}
Let $s$ be a large positive real number ($s \in \mathbb{R}^+$). The behavior is dominated by the linear term:
\begin{equation}
    \Phi_{\text{max}}(s) = s - \sqrt{2s} \approx s
\end{equation}
Thus, the integral exhibits \textbf{exponential growth}:
\begin{equation}
    |I(s)| \sim e^s
\end{equation}

\subsection*{3. Case B: The Imaginary Axis (Fourier Domain)}
Let $s$ be purely imaginary, corresponding to frequency $s = i\omega$. Substituting into the Master Equation:
\begin{equation}
    \Phi_{\text{max}}(i\omega) \approx i\omega - \sqrt{2(i\omega)}
\end{equation}
Using the principal root $\sqrt{i} = \frac{1+i}{\sqrt{2}}$, the square root term becomes:
\begin{equation}
    \sqrt{2(i\omega)} = \sqrt{2\omega} \cdot \frac{1+i}{\sqrt{2}} = \sqrt{\omega}(1+i) = \sqrt{\omega} + i\sqrt{\omega}
\end{equation}
Substituting this back into $\Phi_{\text{max}}$:
\begin{align}
    \Phi_{\text{max}}(i\omega) &\approx i\omega - \left( \sqrt{\omega} + i\sqrt{\omega} \right) \\
    &= -\sqrt{\omega} + i(\omega - \sqrt{\omega})
\end{align}
The magnitude of the integral depends only on the real part of the exponent:
\begin{equation}
    |I(\omega)| \sim \exp\left( \text{Re}[\Phi_{\text{max}}] \right) = e^{-\sqrt{\omega}}
\end{equation}
\textbf{Conclusion:} On the imaginary axis, the linear growth term $i\omega$ 
becomes purely distinct phase oscillation, leaving the $\sqrt{\omega}$ 
term to enforce Gevrey-class decay.

\begin{remark}
We can do similar analysis for functions of the form
\begin{equation}
    I(s) = \int_{-1}^{1} \exp\left( st + \frac{1}{t^{2n} - 1} \right) dt
\end{equation}
and we arrive at the asymptotic expression
\begin{equation}
|I(\omega)| \sim e^{-\sqrt{\frac{ \omega}{n}}}
\end{equation}

\end{remark}

\begin{remark}
This shows us that higher n, the more boxy shapes have not as good spectral decay, but due to the nature of their shapes, they have better SNR.
\end{remark}


\section{The CMST Resolution Law for CMST(2)}

To distinguish two signals with an amplitude ratio $R$, separated by a normalized frequency ratio $\rho$, an instrument with sampling rate $f_s$ requires a minimum observation time $T$. These four variables are governed by the inequality:

\begin{equation}
    \underbrace{T \cdot f_s}_{\text{Total Samples } N} \quad \times \quad \underbrace{\left( \frac{\Delta f}{f_s} \right)}_{\text{Closeness Ratio } \rho} \quad \ge \quad \underbrace{ \frac{\left[ \ln(R) \right]^2}{\pi} }_{\text{Contrast Cost}}
\end{equation}

\subsection{Implications}

\textbf{1. Solving for Time ($T$):}\textit{\textit{•}}
How long must I observe?
\begin{equation}
    T \ge \frac{1}{\pi \Delta f} [\ln(R)]^2
\end{equation}
\textit{Note: Sampling rate $f_s$ cancels out. Time is purely a function of absolute frequency separation and contrast.}

\textbf{2. Solving for Closeness ($\rho$):}
How close can two frequencies be for a fixed $N$?
\begin{equation}
    \rho \ge \frac{[\ln(R)]^2}{\pi N}
\end{equation}
\textit{The resolvable fraction of the bandwidth improves linearly with the total number of samples.}

\textbf{3. Solving for Contrast ($R$):}
What dynamic range can I achieve?
\begin{equation}
    R \le \exp\left( \sqrt{N \cdot \pi \rho} \right)
\end{equation}
\textit{Dynamic range grows exponentially with the square root of the "Resolution Product" ($N \rho$). This is the signature advantage of the Gevrey class.}

Since the number of frequency bins separating two signals is defined as $m = T \cdot \Delta f$, the law can be stated simply in terms of FFT bins:

\begin{equation}
    m \ge \frac{ \ln(R)^2}{\pi}
\end{equation}

\noindent \textbf{4. Practical Rule:} To resolve a weak signal against a contrast ratio $R$, you must observe long enough that the signal shifts at least $\frac{1}{\pi} \ln(R)^2$ bins away from the interferer.
\begin{itemize}
    \item For $80\,\text{dB}$ contrast ($R=10^4$): Separation $\ge \mathbf{27}$ bins.
    \item For $100\,\text{dB}$ contrast ($R=10^5$): Separation $\ge \mathbf{42}$ bins.
    \item For $120\,\text{dB}$ contrast ($R=10^6$): Separation $\ge \mathbf{61}$ bins.
\end{itemize}

\subsection{Numerical Accuracy of amplitude and phase}
For a function $f(t) = A \cos(\omega t) + B \sin(\omega t)$
The Fourier transform at frequency $\omega$  with window w(t) is:

\begin{equation}
\mathcal{F}(\omega) = \int_{-1}^{1} w(t) \left( A \cos(\omega t) + B \sin(\omega t) \right) e^{i \omega t} \, dt
\end{equation}
or
\begin{equation}
\mathcal{F}(\omega) = \int_{-1}^{1} w(t) [A \cos^2(\omega t) + B \sin(\omega t) \cos(\omega t) + i (A \sin(\omega t) \cos(\omega t) + B \sin^2(\omega t))] \, dt
\end{equation}
and assuming w(t) is even gives
\begin{equation}
\text{Re}(\mathcal{F(\omega)}) = \frac{A}{2} \left[ \int_{-1}^{1} w(t) \, dt + \int_{-1}^{1} w(t) \cos(2\omega t) \, dt \right]
\end{equation}
and
\begin{equation}
\text{Im}(\mathcal{F(\omega)}) = \frac{i B}{2} \left[ \int_{-1}^{1} w(t) \, dt - \int_{-1}^{1} w(t) \cos(2\omega t) \, dt \right]
\end{equation}

So the perfect window would give us just this.

\begin{equation}
\mathcal{F(\omega)} = \frac{A}{2} \int_{-1}^{1} w(t) \, dt + i \frac{i B}{2}  \int_{-1}^{1} w(t) \, dt 
\end{equation}
...
is just a constant, independent of frequency, so the error term is
\begin{equation}
\int_{-1}^{1} w(t) \cos(2 \omega t) \, dt
\end{equation}

Which is 
\begin{equation}
\text{Re}(\mathcal{F}(2\omega))
\end{equation}

which, since $\mathcal{F}(s)$ decays super-algebraically with CMST, means that  the CMST is generally more accurate in both phase and amplitude than  windows with algebraic decay.

\subsection{Spectral Stiffness}
(Largley Harrris \cite{Harris1978})
If we consider a signal transform around its center frequency we get
\begin{equation}
F(s) = \int_{-1}^{1} \cos(s+\delta)\cos(s)w(t) \, dt
\end{equation}
Rearranging gives
\begin{equation}
 \int_{-1}^{1} \frac{1}{2} (\cos (\delta +2 s)+\cos (\delta )) w(t) \, dt
\end{equation}
Now since $ \int_{-1}^{1} \frac{1}{2} (\cos (\delta +2 s)) w(t) \, dt \approx -F(2s)/2 $
and since for any reasonable window and $s$ , $F(2s) \ll F(s)$ we get

\begin{equation}
F(s+\delta) \approx \int_{-1}^1 \frac{1}{2} \cos (\delta ) w(t) \, dt
\end{equation}

Expanding the first 3 derivatives gives us
\begin{equation}
F(s+\delta) \approx \int_{-1}^{1} w(t) \, dt (1  -\frac{\mathcal{K^2}}{2} \delta ^2 )
\end{equation}
Where 
\begin{equation}
\mathcal{K} = \sqrt{\frac{\int_{-1}^{1} t^2 w(t) \, dt}{\int_{-1}^{1} w(t) \, dt}}
\end{equation}
which we shall refer to as the Spectral stiffness or the normalised standard deviation of the window.

I believe this is a useful measure in determinig how sharp a window creates peaks from signals.  The higher, the better.

\subsection{Using Spectral Stiffness to bound signal resolution}

To look at the resolution of two signals with the same strength and difference in frequency of $\delta$ we can model the frequency response as a Gaussian.  We then get:

\begin{equation}
G(\delta) = e^{-\frac{\delta^2}{2\sigma^2}} \approx 1 - \frac{\delta^2}{2\sigma^2}
\end{equation}
and from our approximation above this gives us:

\begin{equation}
\frac{\mathcal{K}^{2}}{2} = \frac{1}{2\sigma^2} \implies \sigma = \frac{1}{\mathcal{K}}
\end{equation}
Now, if we assume that for the same amplitude Gaussian we need $2\sigma$ for the peaks to be distinct we get:

\begin{equation}
\delta \ge 2\sigma \implies \delta \ge \frac{2}{\mathcal{K}}
\end{equation}

Which converting into time  of recording, T means:
\begin{equation}
T \ge \frac{2}{{\mathcal{K} \, \delta} }
\end{equation}

\begin{remark}
This is true of the "perfect" transform, not a discrete transform and is a fundamental function of the window itself.  The sample frequency does not matter.  It is an approximation, but I think a reasonable one.
\end{remark}


\subsection{Using Spectral Stiffness to produce "bin limit"}

For a given observation time T we have:
\begin{equation}
T = \frac{2}{\kappa \delta} 
\end{equation}

giving number of samples N as:
\begin{equation}
N = f_s T = \frac{2 f_s}{\kappa \delta} 
\end{equation}
Using a bin width of 
\begin{equation}
\Delta f_{\text{bin}} = \frac{f_s}{N} 
\end{equation}
and a minimum signal separation ability of
\begin{equation}
\Delta F = \delta
\end{equation}
We get that the separation of the minimum resolvable signals is
\begin{equation}
S_{\text{bins}} = \frac{\Delta F}{\Delta f_{\text{bin}}} = \frac{\delta}{\left(\frac{f_s}{N}\right)} = \frac{N\delta}{f_s} 
\end{equation}
giving a final bin resolution limit of
\begin{equation}
S_{\text{bins}} = \frac{2}{\kappa}
\end{equation}

\begin{remark}
Again, $f_s$ does not come into it.  Obviously we would want  $f_s$ to be bigger than twice the frequencies that we wanted to separate for them to appear at the correct frequencies.  
\end{remark}

\begin{remark}
Geometrically, placing the centers of two peaks exactly 2 bins apart requires a frequency separation of precisely $\Delta F = \frac{2}{T} = \frac{2f_s}{N}$.
\end{remark}

\begin{remark}
In reality, however, the spectral leakage of the applied window prevents us from resolving signals at this ideal 2-bin limit. Because our window parameter is $\kappa \le 1$, the true physical minimum resolvable separation is wider, bounded at $\delta_{min} = \frac{2}{\kappa T} = \frac{2f_s}{\kappa N}$. Therefore, while a 2-bin geometric separation is $2/T$, our practical reality restricts us to only resolving signals separated by $2/(\kappa T)$ or greater. This demonstrates the role Spectral Stiffness plays in distinguishing signals with the same amplitude. 
\end{remark}


\subsection{Using Spectral Stiffness to Sharpen Output}
(Possibly new)
To recover spectral resolution lost to the window's main-lobe spread, we model the blurring process as a discrete convolution. Given the Taylor expansion of our window peak $G(\delta) \approx 1 - \frac{\kappa^2 \delta^2}{2}$, the observed value in a frequency bin $n$ can be approximated as the sum of the true signal at that bin and the "leakage" from its immediate neighbors ($\delta = \pm 1$):

\begin{equation}
bin(n) \approx (1 - \kappa^2) signal(n) + \frac{\kappa^2}{2} signal(n-1) + \frac{\kappa^2}{2} signal(n+1)
\end{equation}

By solving this system of equations for $signal(n)$, we derive a discrete 3-tap Laplacian sharpening filter. While the neighbors $bin(n \pm 1)$ also contain components of $signal(n)$, a first-order algebraic inversion yields the following sharpening operator:

\begin{equation}
signal(n) \approx \frac{1}{1 - \kappa^2} \left[ bin(n) - \frac{\kappa^2}{2} \left( bin(n+1) + bin(n-1) \right) \right]
\end{equation}

This filter attempts to reverse the spectral "softness" introduced by the window. Because the Spectral Stiffness $\kappa$ is a deterministic property of the chosen window, this correction is scale-invariant and independent of the sampling frequency $F_s$. In high signal-to-noise ratio (SNR) environments this recovers some of the resolution lost to spectral stiffness.  It should however be noted that applying this filter magnifies noise and there are a couple of approximations in the derivation.  The window shape is not gaussian and our bins are impacted by more than just the adjacent bins.  Its usefulness will depend on the application.




\subsection{Numerical Visualisation: The "Planet Test"}
To visualise the resolution law in a 2D imaging context, we simulated a high-contrast stellar observation.
\begin{itemize}
    \item \textbf{Setup:} A "Sun" (Radius $15\,\text{px}$) and eight "Planets" (at radius $50\,\text{px}$) on a $2048 \times 2048$ grid.
    \item \textbf{Contrast:} The planets were set to $-80\,\text{dB}$ relative to the Sun.
    \item \textbf{Separation:} The effective edge-to-edge separation was $\Delta x = 50 - 15 = 35\,\text{px}$.
\end{itemize}

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\linewidth]{planets.png}
    \label{fig:Planets}
    \caption{\textbf{The "Planet Test"}} 

\end{figure}

\noindent \textbf{Calculation:} According to the CMST Resolution Law for $R=10^4$ ($80\,\text{dB}$), the required separation is:
\begin{equation}
    m \ge \frac{1}{\pi} (\ln 10^4)^2 \approx 27\,\text{bins}
\end{equation}

\noindent \textbf{Result:} Since the available separation ($35\,\text{px}$) exceeded the required separation ($27\,\text{px}$), the CMST window successfully resolved the planets with a clear noise floor margin. A standard polynomial window would require a larger number of bins for this contrast, rendering the planets invisible or obscured by leakage.
...
The two main references are S.~Fisk,  and G.P\'olya. While it is true that when it comes to zeros, it is always P\'olya, I wouldn't be here without Fisk
\begin{thebibliography}{99}

\bibitem{Fisk}
S.~Fisk,
\emph{Polynomials, roots, and interlacing},
arXiv preprint arXiv:math/0612833 2006.
...
\bibitem{Gevrey1918}
M.~Gevrey,
\emph{Sur la nature analytique des solutions des \'equations aux d\'eriv\'ees partielles. Premier m\'emoire},
Ann. Sci. \'Ecole Norm. Sup. (3), vol.~35, 1918, pp.~129--190.

\bibitem{Harris1978}
F.~J.~Harris,
\emph{On the Use of Windows for Harmonic Analysis with the Discrete {Fourier} Transform},
\textit{Proceedings of the {IEEE}}, vol.~66, no.~1, pp.~51--83, Jan.\ 1978.
