...
\begin{remark}
Since the CMST family is closed under multiplication, the energy window is itself a CMST. This implies the Power Spectral Density (PSD) inherits the same super-algebraic decay and resolution properties as the amplitude spectrum, a unique feature not found in polynomial-based windows
\end{remark}
...
\begin{remark}
The resulting FIR filter is not only stable under high-order 
recurrence but also exhibits
a strictly confined bandwidth. 
Because the CMST Window is $C^{\infty}$ and of Gevrey class $\mathcal{G}^2$
the spectral footprint undergoes a root exponential
plunge, allowing for target resolutions 
to be reached 
with a 
minimal tap count.
...
While the super-algebraic property guarantees decay faster than any polynomial $O(\omega^{-N})$, it is useful to derive the precise asymptotic envelope. The window function $\psi(t) = \exp((t^2-1)^{-1})$ belongs to the Gevrey class $\mathcal{G}^s$ with index $s=2$. The bounds on its derivatives satisfy the Gevrey condition:
\begin{equation}
\max_{t \in [-1, 1]} |\psi^{(k)}(t)| \leq C^{k+1} (2k)!
\end{equation}
for some constant $C > 0$. Using the standard bound for the Fourier transform derived via repeated integration by parts:
\begin{equation}
|\hat{\psi}(\omega)| \leq \frac{2 \max |\psi^{(k)}(t)|}{|\omega|^k} \approx \frac{2 C^{k+1} (2k)!}{|\omega|^k}
\end{equation}
...
\begin{equation}
|\hat{\psi}(\omega)| \leq A \exp\left( -B \sqrt{|\omega|} \right)
\end{equation}
...
For the window function $\psi(t) = \exp((t^2-1)^{-1})$, the Fourier transform magnitude behaves asymptotically as:
\begin{equation}
|\hat{\psi}(\omega)| \sim \mathcal{O}\left( |\omega|^{-3/4} \exp\left(-\sqrt{|\omega|}\right) \right) \quad \text{as } |\omega| \to \infty
\end{equation}
This establishes the Gevrey $\cite{Gevrey1918}$ index $s=2$, corresponding to a root-exponential decay rate.

This decay rate is optimal among all compactly supported 
$C^\infty$ functions in the sense that no faster-than-root-exponential 
decay is achievable without sacrificing compact support.

\end{theorem}
\begin{theorem}[Asymptotic Decay Rate]
For the window function $\psi(t) = \exp((t^2-1)^{-1})$, the Fourier transform magnitude satisfies the Gevrey class condition:
\begin{equation}
    |F(i\omega)| \approx C \, |\omega|^{-3/4} \exp\left(-\sqrt{|\omega|}\right) \quad \text{as } |\omega| \to \infty
\end{equation}
This establishes that $\psi(t)$ belongs to the Gevrey class $\mathcal{G}^2$ \cite{Gevrey1918}, corresponding to a root-exponential spectral decay.
\end{theorem}

\begin{remark}
In the discrete domain (FFT), the angular frequency relates to the bin index $m$ by $\omega \approx \pi m$. Substituting this yields the effective decay rate for signal processing:
\begin{equation}
    |W[m]| \sim m^{-3/4} e^{-\sqrt{\pi m}}
\end{equation}
\end{remark}


\section{The CMST Discrimination Formula}

For a Gevrey-class window of index $s$ (where $s=2$ for the CMST), the asymptotic spectral decay is governed by a stretched exponential. To distinguish a weak signal from a strong interfering source with a dynamic range ratio $R$, the window function $W(\omega)$ must decay sufficiently at the frequency separation $\Delta f$.

\subsection{The Gevrey Decay Law}
The spectral magnitude of the CMST window decays as:
\begin{equation}
    |W(\omega)| \approx C e^{-\sqrt{\omega}}
\end{equation}
where $\omega$ is the frequency offset in bins.

\subsection{The Resolution Condition}
Let $R$ be the required linear dynamic range (e.g., $R = 10^5$ for 100 dB). The leakage at distance $\omega$ must satisfy:
\begin{equation}
    e^{ -\sqrt{\omega}} \le \frac{1}{R}
\end{equation}

Solving for the minimum number of frequency bins $m$ required for separation:
\begin{align}
    -\sqrt{\pi m} &\le -\ln(R) \\
    m &\ge \frac{\left( \ln(R) \right)^2}{\pi}
\end{align}

\subsection{The Discrimination Formula}
Relating the bin count $m$ to physical parameters, where $m = \frac{N \cdot \Delta f}{f_s}$:
\begin{equation}
    \frac{N \cdot \Delta f}{f_s} \ge \left( \ln(R) \right)^2
\end{equation}

Solving for the total number of samples $N$:
\begin{equation}
     N \ge \frac{f_s}{\Delta f} \cdot \frac{\left(\ln(R)\right)^2 }{\pi}
\end{equation}

\noindent \textbf{Interpretation:}
Unlike polynomial windows (where $N \propto R^{1/p}$), the CMST cost scales with the \textit{square of the logarithm} of the dynamic range. This allows for high-contrast detection with significantly shorter integration times than standard smooth windows.


\section{The CMST Resolution Law}
