...
\begin{remark}[Relationship of $\phi$]
Noting that if we set $a_0 = 1$ and $a_1 = \phi$ we get

\begin{equation}
\phi = -\frac{\int_{-1}^1 t^2 \exp \left(\frac{1}{t^2-1}\right) \, dt}{2 \int_{-1}^1 \exp
...
\begin{remark}
Note that numeric convergence of the above series is hugely dependant on the accuracy of $\phi$.  A 7 decimal place accurate value for $\phi$ blows up around the 5th term for instance.  However if we work out the terms they do telescope giving for instance the first ten terms as
\begin{equation}
\int_{-1}^{1}  \exp\left(\frac{1}{t^2 -1}\right)\, dt 
\approx -\frac{676457349120}{17\, e \, (741069365504297 \phi +58553612934767)} \approx 0.4439530
\end{equation}
...
\end{theorem}
\begin{theorem}[Asymptotic Decay Rate]
For the window function $\Phi(t) = \exp((t^2-1)^{-1})$, the Fourier transform magnitude satisfies the Gevrey class condition:
\begin{equation}
    |F(i\omega)| \approx C \, |\omega|^{-3/4} \exp\left(-\sqrt{|\omega|}\right) \quad \text{as } |\omega| \to \infty
\end{equation}
This establishes that $\Phi(t)$ belongs to the Gevrey class $\mathcal{G}^2$ \cite{Gevrey1918}, corresponding to a root-exponential spectral decay.
\end{theorem}
