...
\end{theorem}
\begin{proof}
That is simply the result of applying $sD$ , the Euler operator, to the transform, and moving the $t^2$ terms outside to their $\mathcal{T}_{\text{rad}}$ equivalents.
\end{proof}
\begin{remark}
...
We can then take the limit as $a \to 1$
\end{proof}

\begin{lemma}[$e^{\frac{1}{t^2-1}} = \frac{1}{e} \left(e^{\frac{1}{t^2-1}}\right)^{t^2} $]
\end{lemma}
\begin{proof}
We can show this by taking logs of both sides giving
\begin{equation}
\frac{1}{t^2-1} = \frac{1}{t^2-1}{t^2}-1 \to \frac{1}{t^2-1}
\end{equation}
\end{proof}
...
\end{equation}
and $\mathrm{Si}(x)$ is the Cosine Integral
\begin{equation}
...
If we let
\begin{equation}
F(s) = \int_{-a}^a \frac{\cos (s t)}{1-t^2} \, dt
\end{equation}
we get
\begin{equation}
F(s) == -F''(s) + \frac{2 \sin (a s)}{s}
\end{equation}
Which he can solve giving us the general solution
\begin{equation}
F(s)\to \cos (s) (-\text{Ci}((1-a) s)+\text{Ci}((a+1) s)+c_1)+\sin (s)
   (\text{Si}((a-1) s)+\text{Si}((a+1) s)+c_2)
\end{equation}
Noting that the function is even and equating coefficients gives us $c_1 =0$, $c_2 =0$ giving us our result
\end{proof}
