...
\section{The Geometric-Exponential Kernel}
...
$F_n(s) = \mathcal{C}_1[t^{2n}\Phi(t)](s)$ is a globally interlacing family in 
...
\begin{equation}
    \frac{1}{t^{2k}-1} = -\sum_{n=0}^{\infty} t^{2nk} = -(1 + t^{2k} + t^{k4} + \dots)
\end{equation}
The kernel can be expressed as $e^{-f(t^2)}$ where $f(t) = \sum_{n=0}^{\infty} a_n t^n$ with $a_n = 1$ for all $n$. 
\end{proof}

\begin{remark}
The $e^{t^{2n}}$ operators are benign and in $LP$ so we can flatten this function with 
\[
e^{t^{2 k}+\frac{1}{t^{2 k}-1}}
\]
